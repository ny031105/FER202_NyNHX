...
function News() {
  ...
  return (
    <Container className="py-3">
      <h1 className="news-title mb-4">News Category</h1>
      
      <div className="news-grid">
        {newLists.map((news) => (
          <Card key={news.id} className="news-card h-100">
            <Card.Img
              variant="top"
              src={news.images}
              alt={news.title}
            ...
            <Card.Body className="d-flex flex-column justify-content-between">
              <div>
                <Card.Title className="fw-bold fs-5">{news.title}</Card.Title>
                <Card.Text className="text-muted small">{news.description}</Card.Text>
              </div>
              <div className="mt-3">
                <Link to="" className="news-link-bottom small">
                  {news.title}
                </Link>
              </div>
            </Card.Body>
